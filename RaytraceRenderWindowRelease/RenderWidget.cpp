...
void RenderWidget::paintGL()
    ...
    if (renderParameters->useLighting)
        ...
        glLightfv(GL_LIGHT0, GL_DIFFUSE,    renderParameters->lightColor);
        glLightfv(GL_LIGHT0, GL_SPECULAR,   renderParameters->lightColor);

        //Attenuation will not do anything if light is directional. If we change, this is what we want (quadratic only attenuation)
        glLightf(GL_LIGHT0, GL_CONSTANT_ATTENUATION, 0.0f);
        glLightf(GL_LIGHT0, GL_LINEAR_ATTENUATION, 0.0f);
        glLightf(GL_LIGHT0, GL_QUADRATIC_ATTENUATION, 1.0f);

//        float globalAmbientColour[4] = {1.0f,1.0f,1.0f,1.0f};
        float globalAmbientColour[4] = { 0.0f, 0.0, 0.0, 1.0 };
